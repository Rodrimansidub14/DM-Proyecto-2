...
pca_data <- train_data %>% select(one_of(pca_vars)) %>% na.omit()
...
cat("Determinante de la matriz de correlación:", round(det_cor, 5), "\n")
...
Separamos los datos en conjuntos de entrenamiento y testeo, y ajustamos un modelo de regresión lineal para predecir el precio de venta de las viviendas.
...
Trabajamos con la variable más correlacionada con el precio de venta: GrLivArea.
...
# Imprimir el número total de predicciones
...
tabla_pred_uni <- data.frame(Predicciones = head(pred_uni))
kable(tabla_pred_uni, caption = "Primeras Predicciones del Modelo Univariado")

# Mostrar el RMSE del modelo
tabla_rmse <- data.frame(Métrica = "RMSE", Valor = rmse_uni)
kable(tabla_rmse, caption = "Métricas del Modelo Univariado")
